...
<?php get_header(); ?>
	...
	<div id="prev_hover_arrow" style="display:none;">Prev image</div>
	<div id="next_hover_arrow" style="display:none;">Next image</div>
	...
	<div style="display:block;position:fixed;bottom:5%;right:0;z-index:1000;margin:0;width:960px;" class="right">
		<div class="buttons left_rounded">
			<img title="View project info" id="info_window" src="http://www.firedog.co.uk/wp-content/themes/Firedog/images/info_button.png"><br>
			<?php
			...
			?>
			<a title="Back to creative results" id="back_to_filter" href="<?php echo $back_link;?>"><img src="http://www.firedog.co.uk/wp-content/themes/Firedog/images/results.png"></a>
		</div>
		<div id="info_window_block" class="padding_full left_rounded" style="display:none;background:black;">
			<?php
			$client = current($client);
			?>
			<h1 class="port_header"><?php echo $client->name;?><span class="pipe">|</span><?php the_title(); ?></h1>
			<div class="row one">
				<h4>Tag Info</h4>
				<ul class="port_tag">
				...
				<?php
				$download_pdf = get('download_pdf');
				$post_meta_pdf = false;
				if( $download_pdf == "" ){
					$download_pdf = get_post_meta($post->ID,'download_pdf', true);
					$post_meta_pdf = true;
				}
				
				if( $download_pdf != "" AND $post_meta_pdf)
					$download_pdf = get_bloginfo('siteurl').'/wp-content/files_mf/'.$download_pdf;
				
				if( $download_pdf != ""):?>
				<div class="pdf_top">
					<p><a target="_blank" href="<?php echo $download_pdf; ?>" title="Download a PDF">Download a PDF case study to take away for later</a></p>
			...
			<div class="row one">
				...
				<?php echo $img;?>
				<h6 class="port_header_mini"><?php echo $client;?></a> <span class="pipe">&#124;</span> <a href="<?php the_permalink();?>"><?php the_title(); ?></h6>
				</a>
				<br /><br />
	...
	<div id="galleria">
		...
		if($c==1 AND $prev_case_study_url AND count($cookie_filter_cs_ids))
			echo "<img src='http://www.firedog.co.uk/wp-content/uploads/prev_case_link.jpg' alt='' title='' >\n";
		elseif($c==1 AND count($cookie_filter_cs_ids))
			echo "<img src='http://www.firedog.co.uk/wp-content/uploads/home_case_link.jpg' alt='' title='' >\n";
		**/
		...
		foreach( $fullscreens_order as $i ){
			if ( MultiPostThumbnails::has_post_thumbnail('case_studies', 'fullscreen-image-'.$i) ) :
				...
				if ($img_attachment) {
						...
						(
							...
							[post_parent] => 275
							[guid] => http://www.firedog.co.uk/wp-content/uploads/2011/05/aristocrat_2_600_390.jpg
							[menu_order] => 0
						...
						**/
						echo "<img src='".$img_attachment->guid."' alt='".$img_attachment->post_content."' title='<h4>".$img_attachment->post_excerpt."</h4>' >\n";
				}
			endif;
			...
			if( $c==20 AND $next_case_study_url AND count($cookie_filter_cs_ids) )
				echo "<img src='http://www.firedog.co.uk/wp-content/uploads/next_case_link.jpg' alt='' title='' >\n";
	...
	**/
	...
	//LEFT
	var prev_hover_arrow = jQuery('#prev_hover_arrow');//prev hover button near mouse	
	jQuery(".galleria-image-nav-left").live("mouseover click",function(event){
		prev_hover_arrow.show();
		...
		if(prev_button != undefined){
			if( prev_button.indexOf('prev_case_link') != -1 || prev_button.indexOf('home_case_link') != -1){
				//prev_hover_arrow.html('Prev project');
			}else{
				prev_hover_arrow.html('Prev image');
			}
			if(event.type == 'mouseover'){
				if ( browser.msie && browser.version.slice(0,3) < 9 )
					prev_hover_arrow.stop().show();
				else
					prev_hover_arrow.stop().animate({opacity:'1'},800);
			}
		}
    });
	jQuery(".galleria-image-nav-left").live("mousemove mouseout",function(event){
		...
		if(event.type == 'mouseout'){
			if ( browser.msie && browser.version.slice(0,3) < 9 )
				prev_hover_arrow.stop().hide();
			else
				prev_hover_arrow.stop().animate({opacity:'0'},800);
		}
		prev_hover_arrow.css({top:event.pageY + offsetY+ "px", left: event.pageX + offsetX +"px"});
		
    });
	//RIGHT
	var next_hover_arrow = jQuery('#next_hover_arrow');//next hover button near mouse
	jQuery(".galleria-image-nav-right").live("mouseover click",function(event){
	
		next_hover_arrow.show();
		...
		if(next_button != undefined){
			...
			if( next_button.indexOf('next_case_link') != -1 || next_button.indexOf('home_case_link') != -1){
				//next_hover_arrow.html('Next project');
			}else{
				next_hover_arrow.html('Next image');
			}
			if(event.type == 'mouseover'){
				if ( browser.msie && browser.version.slice(0,3) < 9 )
					next_hover_arrow.stop().show();
				else
					next_hover_arrow.stop().animate({opacity:'1'},800);
			}
		}
    });
	...
	jQuery(".galleria-image-nav-right").live("mousemove mouseout",function(event){
		...
		if(event.type == 'mouseout'){
			if ( browser.msie && browser.version.slice(0,3) < 9 )
				next_hover_arrow.stop().hide();
			else
				next_hover_arrow.stop().animate({opacity:'0'},800);
		}
		next_hover_arrow.css({top:event.pageY + offsetY + "px", left: event.pageX + offsetX +"px"});
    });
	...
	/** thumbs to redirect to another case studies
	function redirection_behavior() {
	...
	jQuery(".galleria-image, .galleria-image-nav, #prev_hover_arrow, #next_hover_arrow").live('click', function () {
		redirection_behavior();
	});
